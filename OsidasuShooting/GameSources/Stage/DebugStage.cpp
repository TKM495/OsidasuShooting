...
namespace basecross {
	...
	void DebugStage::OnCreate() {
		try {
			...
			Debug::GetInstance()->Log(L"M → Main");
			Debug::GetInstance()->Log(L"W → WatanabeStage");
			Debug::GetInstance()->Log(L"P → WatanabeStage2");
	...
	void DebugStage::OnUpdate() {
		...
		else if (keyState.m_bPressedKeyTbl['P'])
			PostEvent(0.0f, GetThis<ObjectInterface>(), app->GetScene<Scene>(), L"ToWatanabeStage2");
	}
}