...
namespace basecross {
	class CharacterSelectStage :public Stage {
		...
		int m_loopForPlayer;	// for?Å˜?A?v???C???[?I?l?Åh?aÅgu?e?A?Å·
		int m_gamePadIDs[4];
		...
		shared_ptr<CharacterIcon> m_Icons[12]; // ?A?C?R?Åg3 * ?v???C???[?Åh
		shared_ptr<SelectTriangle> m_Triangle[8]; // ?O?p2 * ?v???C???[?Åh
